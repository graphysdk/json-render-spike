 ...
 *
 * The chart carries no chart-type enum: its `spec` prop is a viz-engine `SpecInput` — an aesthetic
 * mapping, a stack of geom layers, and the scales those layers are positioned by, read through a
 * coordinate system. Grouped bars are `bar` with `position: 'dodge'`; a donut is `bar` under polar
 * coords. That keeps the catalog entry small enough to sit in a prompt beside every other component,
