...
function buildChartDescription(): string {
  ...
  return [
    ...
    'RESTYLING (`spec.styles`): when the user asks for specific colors or label styling, author stylesheet entries — {"overrides":[{"select":{"target":"geom","kind":"bar"},"declarations":{"color":"#D72B1C"}}]}. Targets: geom (optionally with kind), dataLabel, axisLabel, tickLabel, graph. Declarations: color, alpha, borderColor, borderWidth, borderRadius ("none"…"full"), strokeWidth, lineType, fillAlpha, size, background, textColor, fontSize, fontWeight. `defaults` fill what no color scale decided; `overrides` replace even scale-assigned paint, and both layer over a baked `style`. A geom entry takes `when` to target observations — {"when":{"where":{"variable":"month","eq":"Jan"}}}, operators eq|oneOf|lt|lte|gt|gte. Never fake paint through constant color mappings or palette overrides.',
    'EMPHASIS (`spec.highlights`): to call out part of the data without recoloring it, add [{"type":"highlight","predicate":{"variable":"region","eq":"EU"},"scope":"series"}] — matched marks keep full paint, the rest dim. Scope: "data-point" (default, per row), "series" (the whole group), "x-value" (a vertical slice).',
    'NOTES (`spec.annotations.textAnnotations`): a short note pinned on the plot — [{"text":"Holiday spike","at":{"anchorType":"observation","anchorValue":"Dec"}}] pins to a data point by its x value (add "groupValue" to name the series); {"anchorType":"panel","x":0.8,"y":0.2} pins at plot fractions. Use when the user wants words on the chart itself.',
  ].join(' ');
}
