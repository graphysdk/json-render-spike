...
function buildChartDescription(): string {
  ...
  return [
    ...
    'NOTES (`spec.annotations.comments`): a comment pinned to a data point — [{"text":"Holiday spike","at":{"anchorValue":"Dec"}}] marks the observation with that x value (add "groupValue" to name the series) and reveals the note on hover. Use when the user wants words on the chart itself.',
  ].join(' ');
}
